...
function autoexec main()
{
	...
	clientfield::register( "actor", "mechz_faceplate_detached", VERSION_DLC1, 2, "int", &MechzClientUtils::mechz_detach_faceplate, !CF_HOST_ONLY, !CF_CALLBACK_ZERO_ON_NEW_ENT );
	...
	clientfield::register( "actor", "mechz_claw_detached", VERSION_DLC1, 2, "int", &MechzClientUtils::mechz_detach_claw, !CF_HOST_ONLY, !CF_CALLBACK_ZERO_ON_NEW_ENT );
...
{
	if( IsDefined( level.mechz_detach_faceplate_override ) )
	{
		self [[level.mechz_detach_faceplate_override]]( localClientNum, oldValue, newValue, bNewEnt, bInitialSnap, fieldName, wasDemoJump );
		return;
	}
