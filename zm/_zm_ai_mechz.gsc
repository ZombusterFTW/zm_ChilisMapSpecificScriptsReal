...
function mechz_melee_knockdown_function_override()
{
	...
	foreach ( e_zombie in a_zombies )
	{
		...
		if ( e_zombie mechz_knockdown_zombie_valid( self ) && n_distance_sq <= 12544 )
			self mechz_knockdown_zombie( e_zombie );
...
function mechz_health_increases()
{
	if ( !isDefined( level.mechz_last_spawn_round ) || level.round_number > level.mechz_last_spawn_round )
	{
		...
		n_player_modifier = 1;
		if ( a_players.size > 1 )
			n_player_modifier = a_players.size * .75;
		
		level.mechz_health = int( n_player_modifier * level.mechz_base_health + level.mechz_health_increase * level.mechz_round_count ); // HARRY
