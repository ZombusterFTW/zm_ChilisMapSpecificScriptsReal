...
function fan_trap_effect( n_local_client_num, n_old_val, n_new_val, b_new_ent, b_initial_snap, str_field_name, b_was_time_jump )
{
	...
	if ( IS_TRUE( n_new_val ) )
	{
		...
		{
			self.a_fan_trap = [];
			self.a_fan_trap[ 0 ] = playFxOnTag( n_local_client_num, level._effect[ FAN_TRAP_FX ], self, "fan_rt_jnt" );
			self.a_fan_trap[ 1 ] = playFxOnTag( n_local_client_num, level._effect[ FAN_TRAP_FX ], self, "fan_lft_jnt" );
		}
	}
	else
	{
